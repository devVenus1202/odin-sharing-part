...
interface Props {
  ...
  filterBy: 'MANDATE' | 'PAYMENT' | 'REFUND' | 'BILLING_REQUEST'
}
...
class GoCardlessActivityFeed extends React.Component<Props, State> {
  ...
  private async fetchGoCardlessActivity() {
    ...
    if (record) {
      ...
      } else if (filterBy === 'BILLING_REQUEST') {
        query = `billing_request=${getProperty(record, 'ExternalRef')}`
      }
  ...
  render() {
    ...
    return (
      <div className="gocardless-activity-data-table-wrapper">
        <Card
          className="table-card"
          size="small"
          title={'Gocardless Activity'} style={{ minWidth: 360 }}>
          <Table
            pagination={false}
            size="small"
            loading={isLoading}
