...
import { closeRecordForm, initializeRecordForm, updateFormInput } from '../store/actions';
...
class EmbeddedForm extends React.Component<Props, State> {
  ...
  componentDidMount() {
    const { schema, getPipeline } = this.props;
    if(schema) {
      getPipeline({ schema });
    }
  }
  ...
  handleSubmit = async () => {
    const { createRecord, updateRecord, updateRecordAssociation, logVisit, notifyError, recordFormReducer, onSubmitEvent, closeForm, userReducer } = this.props;
  ...
  constructFormFields = (schema: SchemaEntity | undefined, isRequierdField: boolean, columnPostition: number) => {
    ...
    const { disabledFields, visibleFieldOverride, selected } = recordFormReducer;
  ...
  render() {
    const { schema, isCreateRecord, recordFieldsDisabled } = this.props;
