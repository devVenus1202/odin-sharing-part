  ...
  handleOk = async () => {
    ...
    if(this.state.title && this.state.title.length < 8) {
      ...
      await httpPost(
        ...
        alertMessage({ body: error?.message || 'error saving your view', type: 'error' });
      });
  ...
  render() {
    ...
    return (
      <>
        ...
        <Button onClick={() => {this.setCurrentView(); this.setState({ visible: true })}} icon={<SaveOutlined/>}/>
        <Modal
          title="Save List"
