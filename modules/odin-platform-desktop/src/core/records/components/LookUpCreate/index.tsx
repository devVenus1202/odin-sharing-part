...
class LookUpCreate extends React.Component<Props, State> {
  ...
  fetchData() {
    ...
    const { moduleName, record, entityName, getAssociations, getSchema, setSchemaAssociationData, lookupCreateComponent, schemaReducer } = this.props;
    ...
    getSchema({ moduleName: moduleName, entityName: entityName }, (schema: SchemaEntity) => {
      ...
      if(entityName === 'Premise') {
        ...
        getSchema({ moduleName: moduleName, entityName: entityName }, (res: SchemaEntity) => {
          ...
          const addressSchema = getSchemaFromShortListByModuleAndEntity(schemaReducer.shortList, moduleName, 'Address');
          if(!addressSchema) {
            getSchema({ moduleName: moduleName, entityName: 'Address' })
          }
  ...
  renderRelatedRecordsList = () => {
    const { recordAssociationReducer, entityName, recordReducer, lookupCreateComponent } = this.props;
    return (
      <>
        <List
          ...
          dataSource={
            entityName === 'Premise'
              ? (recordReducer?.list[this.state.premiseSchema?.id] ? recordReducer?.list[this.state.premiseSchema?.id] : [])
              : (lookupCreateComponent && this.state.schema ? recordReducer?.list[this.state.schema.id] : recordAssociationReducer?.list as any[])}
  ...
  render() {
    const { record, entityName, recordAssociationReducer, moduleName, lookupCreateComponent } = this.props;
    return (
      <div>
        <OdinFormModal
          formUUID={uuid}
        ...
        <Row>
          <div style={{ width: '100%' }}>
            ...
            {(recordAssociationReducer?.search !== null && recordAssociationReducer?.search?.terms !== '*' && entityName !== ADDRESS) || lookupCreateComponent ?
