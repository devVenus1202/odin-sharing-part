...
function* getByModuleAndEntity(action: { params: { schema: SchemaEntity, }, cb: any }) {
  ...
  try {
    ...
    console.log('getByModuleAndEntity', res.data.data)
    yield put({ type: GET_PIPELINES_BY_MODULE_AND_ENTITY_SUCCESS, results: res.data.data, schemaId: action.params.schema.id });
    if (action.cb && res.data.data) {
...
function* rootSaga() {
  ...
  yield takeEvery<any>(GET_PIPELINES_BY_MODULE_AND_ENTITY_REQUEST, getByModuleAndEntity);
