...
class LookUpDrawer extends React.Component<Props, State> {
  ...
  handleOk = () => {
    const { record, relation, createAssociations, relationType } = this.props;
    const { schema, schemaAssociation } = relation;
    if (schemaAssociation && record) {
      const associtationEntities = schemaAssociation.label?.split("__")
      const isSelfAssociation = associtationEntities?associtationEntities[0] === associtationEntities[1]:false;
              ...
              actions={[
                ...
                <Popover
                  ...
                  content={
                    ...
                    >
                      <RecordProperties columnLayout="horizontal" record={item} columns={1}/>
                    </div>}>
                  Details
                </Popover>,
              ]}
  ...
  render() {
    ...
    return (
      <div>
        <Button
          type="text"
          disabled={!canUserSearchRecord(userReducer, schema)}
