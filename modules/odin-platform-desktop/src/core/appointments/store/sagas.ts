...
function* loadAvailableAppointments(action: { params: ILoadAppointments }) {
  const { start, end, type, addressId, exPolygonId, isOverview } = action.params;
...
function* cancelAppointment(action: { params: any, cb: any }) {
  try {
