 ...
 * @param entityName
 */
export const canUserAccessModuleAndEntityInRoutes = (routingStructure: any, moduleName: string, entityName: string) => {

  if (routingStructure) {

    const foundModule = routingStructure.find((module: any, i: number) => module.moduleName === moduleName)
...
export const canUserSearchRecord = (authUser: any, schema: SchemaEntity | SchemaEntityTransform | undefined) => {
 ...
 */
 export const isExternalUser = (authUser?: any) => {
  ...
  if (!!authUser && !!authUser.roles && authUser.roles.length > 0 && authUser.user.contactId) {
...
export const hasPermissions: (authUser: any, ...permissions: string[]) => boolean =
  (authUser: any, ...permissions: string[]): boolean => {
...
(authUser: any, ...roles: string[]): boolean => {
  if (!authUser?.roles) {
