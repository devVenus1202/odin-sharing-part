...
import NetworkManageDrawer from '../NetworkManageDrawer';
import NetworkRestartModal from "../NetworkRestartModal";

const { CUSTOMER_DEVICE_ONT, CUSTOMER_DEVICE_ROUTER, PRODUCT, WORK_ORDER, OFFER, PRICE_BOOK } = SchemaModuleEntityTypeEnums;
...
class OrderItemDetailCardView extends React.Component<Props, IState> {
      ...
      <div>
        <div>
          ...
          <div style={{ display: 'flex', marginBottom: 24 }}>
              ...
              <div style={{ marginRight: 12 }}>
                <NetworkRestartModal record={relatedRecord}/>
              </div>
          </div>
          }
        </div>
  ...
  renderDynamicAssociations() {
    ...
    if(showNetworkTab(relatedRecord)) {
      return {
        ...
        Offer: <AssociationDataTable
          title={OFFER}
          record={relatedRecord}
          moduleName={PRODUCT_MODULE}
          entityName={OFFER}/>,
        PriceBook: <AssociationDataTable
          title={'Price Book'}
          record={relatedRecord}
          moduleName={PRODUCT_MODULE}
          entityName={PRODUCT}
          nestedEntityName={PRICE_BOOK} />,
      }
    } else if(showVoiceTab(relatedRecord)) {
      return {
        Summary: <Summary record={record} relatedRecord={relatedRecord}/>,
  ...
  private renderOrderCards() {
    ...
    const link = `/${relatedRecord?.entity?.replace(':', '/')}/${relatedRecord?.id}`
    const defaultActiveKey = this.renderTabsList();
    return <CardWithTabs
      title={<Link to={link}>{`${relatedRecord?.recordNumber} - ${relatedRecord?.title}`}</Link>}
      extra={!disableListActions ? this.getListItemActionMenu(relatedRecord, entityName) : <div/>}
      defaultTabKey={defaultActiveKey[0]?.key}
  ...
  render() {
    ...
    return (
      <>
        <NetworkManageDrawer record={record}/>
