...
import AssociationCardWithTabsList from '../../core/recordsAssociations/components/AssociationCardWithTabsList';
...
class HtmlRecordDetailView extends Component<Props, State> {
  ...
  handleUpdateRecord = () => {
    ...
    record.properties['HTMLContent'] = draftToHtml(convertToRaw(editorState.getCurrentContent()));
    record.properties['JSONContent'] = JSON.stringify(convertToRaw(editorState.getCurrentContent()));
    updateRecord({
      schema,
      recordId: record.id,
      createUpdate: record
  ...
  onEditorStateChange = (state: EditorState) => {
    this.setState({ editorState: state, isChanged: true })
  }
  ...
  render() {
    const { isEditing, editorState, isChanged } = this.state;
    ...
    console.log("html detail record", record);
          ...
          <Col xs={24} sm={24} md={24} lg={6}>
            <div className="record-detail-left-panel">
              <DetailPanelLeft record={record} >
                ...
                <AssociationDataTable title={schema.entityName} record={record} moduleName={schema.moduleName} 
              ...
              <Card
                ...
                {isEditing &&
                  <Editor
                    ...
                    editorStyle={{ border: '1px solid #F1F1F1', minHeight: '800px' }}
                    editorRef={(ref: any) => { }}
                  />}
