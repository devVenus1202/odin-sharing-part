...
class RolesListView extends React.Component<Props, State> {
  state = {
    searchKey: ''
  }
  ...
  renderRoles() {
    ...
    const filteredRoles = dataSource.filter((item: any) => searchString(item.name, this.state.searchKey));
    ...
    return (
      <>
        ...
        <Table
          ...
          style={{ minHeight: '100%' }}
          pagination={false} dataSource={filteredRoles} columns={columns}/>
      </>
    );
  }
