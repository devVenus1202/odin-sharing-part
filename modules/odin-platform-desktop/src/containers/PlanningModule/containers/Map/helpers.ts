...
export const loadMapSidebarLayerSymbology = (layerTitle:string, symbolImageName:string) => {
  return `<img src="../qgis-symbols/${symbolImageName}.png" style="width:14px; height:14px; margin-right:4px" alt="${layerTitle} image" />${layerTitle}`
}

export const buildVisibleLayersSidebar = (visibleLayers:Array<string>) => {

  /* TODO: Get all visible layer names - check against the JSON spec sheet and return back map legend for unique visible layers. */
 ...
 * @param query
 */
export function parseMapQuery(query: string | undefined): { type: string, featureId?: string, postcode?: string, noiRef?: string } | undefined {
