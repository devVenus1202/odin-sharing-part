...
class SchemaColumnDetailView extends React.Component<PropsType, State> {
  ...
  addEnumOption() {
    ...
    const existingOption = selected?.options?.map(elem => ({ id: elem.id, position: elem.position, label: elem.label, value: elem.value, description: elem.description }));
          ...
          {schema?.associations.map((association: SchemaAssociationEntity) => (
            <Select.Option key={association.id} value={association.id}>
              {association.parentSchemaId !== schema.id
                ? association?.parentSchema?.entityName
  ...
  renderSchemaColumnForm() {
    ...
    this.formRef.current?.setFieldsValue({
      ...
      options: selected?.options?.map(elem => ({ id: elem.id, position: elem.position, label: elem.label, value: elem.value, description: elem.description })),
    });
      ...
      <Form layout={'vertical'}
        ...
        <Row gutter={12}>
          ...
          <Col lg={{ span: 12, offset: 7 }} xs={{ span: 24, offset: 2 }}>
            ...
            {
              selected?.type === SchemaColumnTypes.LOOKUP ? this.renderLinkedSchemaAssociationSelect() : ''
            }
            <Form.Item
              className="form-item"
              label="Validators"
            ...
            >
              <Input placeholder="Default format"
            ...
            >
              <Select onChange={(value) => updateProperties({ valueCase: value })} allowClear>
                {titleCaseList.map((it: any) => <Option value={it.value}>{it.label}</Option>)}
              </Select>
            </Form.Item>
            <Form.Item
              className="form-item"
            ...
            <div className="columns-section-wrapper">
              ...
              <Row>
                ...
                <Col span={8}>
                  ...
                  <Form.Item
                    className="form-item"
                    name="isBulkUpdatable"
                    key={21}
                  >
                    <Checkbox checked={selected?.isBulkUpdatable}
                              onChange={(e) => updateProperties({ isBulkUpdatable: e.target.checked })}>Is
                      Bulk
                      Updatable</Checkbox>
                  </Form.Item>
                </Col>
                ...
                <Row key={field.key} gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
                  ...
                  <Col span={8}>
                    <Form.Item
                      ...
                      label="Description"
                    >
                      <Input placeholder="Description" onChange={(e) => this.handleEnumOptionChange(e, index)}/>
                    </Form.Item>
                  </Col>
                  <Col span={3}>
                    <Form.Item
                      ...
                      label="Position"
                    >
                      <Input placeholder="Position" onChange={(e) => this.handleEnumOptionChange(e, index)}/>
                    </Form.Item>
                  </Col>
