  ...
  map?.once('postrender', function () {
    ...
    Array.prototype.forEach.call(
      document.querySelectorAll(
        '.ol-layer canvas, .feedChainMap canvas, .overviewLayer_chains canvas, .overviewLayer_polygon canvas, .googleLayer canvas, .loopChainOverview canvas'),
      function (canvas) {
        if (canvas.width > 0) {
...
export const generateBase64ImageFromMap = async (map: any) => {
  ...
  return await new Promise(async (resolve) => {
    ...
    map?.once('rendercomplete', async () => {
      ...
      Array.prototype.forEach.call(
        document.querySelectorAll(
          '.ol-layer canvas, .feedChainMap canvas,.overviewLayer_polygon canvas, .constructionLayer canvas, .cableAccessLayer canvas, .cableFeedLayer canvas, .googleLayer canvas, .loopChainOverview canvas'),
        function (canvas) {
          if (canvas.width > 0) {
            let opacity = canvas.parentNode.style.opacity;
            // @ts-ignore
