...
import { QGIS_SERVER_BUILDPACK, REACT_APP_QGIS_SERVER_URL } from "./types";
...
class L1BuildPackSpliceless extends React.Component<Props, State> {
      ...
      this.setState({
        ...
        this.loadPolygonAndFeatures(match.params.polygonId).then(() => {
          ...
          this.loadPolygonCableConnections(match.params.polygonId).then(() => {
            ...
            this.loadL1OverviewMap(match.params.polygonId)
  ...
  async loadPolygonCableConnections(polygonId: number) {
    ...
    return await httpGet(
      ...
      this.setState({
        ...
        cableDiagramChains: getChainFromSplicelessData(res.data.data, 'L1', 'L2')
      })
   ...
   * @param polygonId
   */
  async loadL1OverviewMap(polygonId: number) {
    ...
    return await httpGet(
      `ProjectModule/v1.0/ftth/polygon/L1Features/${polygonId}`,
    ).then(res => {

        if (res.data.data && this.state.cableDiagramData) {

          this.setState({ L1FeaturesInPolygon: res.data.data })

          const filteredClosures = this.state.cableDiagramData?.nodes.filter((node: any) => [ 'L0', 'L1', 'L2' ].includes(
            node.type)).map((node: any) => node.id)
          const filteredCables = this.state.cableDiagramData?.edges.filter((edge: any) => filteredClosures.includes(edge.target)).map(
            (edge: any) => edge.id)

          let WMSFilterQuery = `L1_cable_no_scale:"id" IN ( ${filteredCables.join(' , ')} )`
          WMSFilterQuery += `;L1_closure_no_scale:"id" IN ( ${filteredClosures.join(' , ')} )`
          ...
          map?.addLayer(
            new ImageLayer({
              ...
              source: new ImageWMS({
                ...
                crossOrigin: 'anonymous',
                params: {
                  'LAYERS': [ 'L1_cable_no_scale', 'L1_closure_no_scale' ],
      ...
      console.log('%cThere was an error loading L1 features.', 'color:red', err)
    });
  ...
  generateAndExportLoopChainMaps = async () => {
    ...
    const { cableDiagramData, cableDiagramChains, L1FeaturesInPolygon } = this.state
    const { mapReducer } = this.props
    const { map } = mapReducer

    let cableMaps: Array<any> = []
    let piaMaps: Array<any> = []

    this.cleanMapLayers([
      'loopChainOverview',
      'overviewLayer_polygon'
    ])


    if (cableDiagramData && cableDiagramChains && L1FeaturesInPolygon) {
      ...
      for (let j = 0; j < cableDiagramChains.length; j++) {
        ...
        /* TODO: Get the bounding box zoom into it */
        ...
        const allL2PolygonIds = L1FeaturesInPolygon.L2polygons.map((polygon: any) => polygon.id)

        let WMSFilterQuery = `L1_cable_no_scale:"id" IN ( ${allCableIds.join(' , ')} )`
        WMSFilterQuery += `;L1_closure_no_scale:"id" IN ( ${allClosureIds.join(' , ')} )`
        WMSFilterQuery += `;polygon:"id" IN ( ${allL2PolygonIds.join(' , ')} )`
        ...
        map?.addLayer(
          new ImageLayer({
            ...
            source: new ImageWMS({
              ...
              crossOrigin: 'anonymous',
              params: {
                'LAYERS': [ 'L1_cable_no_scale', 'L1_closure_no_scale', 'polygon' ],
                'FILTER': WMSFilterQuery
              },
              ratio: 1,
              serverType: 'qgis',
  ...
  renderPDF = async () => {
    ...
    const pdfData = {
      ...
      L1FeaturesInPolygon: this.state.L1FeaturesInPolygon ? this.state.L1FeaturesInPolygon : [],
  ...
  checkIfCanRun = () => {
    return !(!this.state.L1FeaturesInPolygon ||
            ...
            <Col span={18}>
...

              {
                this.state.diagramRunning ?
                  <NetworkDiagramL1
                    L1FeaturesInPolygon={this.state.L1FeaturesInPolygon!}
            ...
            <Col span={6}>
              <Card className="buildPackSidebar" title={
                ...
                >
                  ...
                  <Descriptions.Item label="L1 &rarr; L2 Chains">
...
export default withRouter(connect(mapState, mapDispatch)(L1BuildPackSpliceless))
