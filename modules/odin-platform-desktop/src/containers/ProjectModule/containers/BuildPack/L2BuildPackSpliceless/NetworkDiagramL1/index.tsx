...
const NetworkDiagramL1 = (props: Props) => {
  ...
  const generateGraphImages = async (scope: 'ALL_CHAINS' | 'SINGLE_CHAIN') => {
    ...
    if (scope === 'ALL_CHAINS' && cableDiagramData) {
      ...
      const L2toL3Chains = getChainFromSplicelessData([ cableDiagramData ], 'L2', 'L3')

      allClosureIds.push(L2toL3Chains[0].parentNode.id)
      L2toL3Chains[0].childNodes.forEach((child: any) => {
