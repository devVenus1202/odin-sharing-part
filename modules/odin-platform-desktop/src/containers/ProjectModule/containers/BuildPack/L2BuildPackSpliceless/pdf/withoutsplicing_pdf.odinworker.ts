...
ctx.addEventListener("message", (arg: any) => {
  ...
  for (let i = 0; i < pageLimit; i++) {
    ...
    doc.setDrawColor(0, 0, 0)
    doc.setFontSize(25)
    doc.text(`L3 -> L4 CHAIN ${(i + 1)}`, 11.3, 10.5)
    addPageNum(doc)

    /* b) L3->L4 Overview Map */
    doc.addPage()
    doc.setDrawColor(0, 0, 0)
    doc.setLineWidth(0.04)
    doc.addImage(cableOverviewMaps[i], 'PNG', 0.4, 0.4, 28.8, 20.2, undefined, "FAST")
    doc.rect(0.4, 0.4, 28.8, 20.2)
    drawLegend(dynamicMapLegend('cable_overview', cableMaps![i]), doc)
    addPageNum(doc)

    infoBox({
      MapType: 'L3 > L4 Overview',
    ...
    /* c) L3 -> L4 Cable Map + Openreach */
    doc.addPage()
    doc.setDrawColor(0, 0, 0)
    doc.setLineWidth(0.04)
    doc.addImage(cablePiaMaps[i], 'PNG', 0.4, 0.4, 28.8, 20.2, undefined, "FAST")
    doc.rect(0.4, 0.4, 28.8, 20.2)
    drawLegend(dynamicMapLegend('cable_or', cableMaps![i]), doc)
    addPageNum(doc)

    infoBox({
      MapType: 'L3 > L4 Cable + OR',
      AccessChainNo: `${i + 1} of ${cableMaps.length}`,
      Author: documentInformation.Author,
      Contact: documentInformation.Contact,
      Date: documentInformation.Date,
    ...
    doc.rect(0.4, 0.4, 28.8, 20.2)
    drawLegend(dynamicMapLegend('openreach', cableMaps![i]), doc)
    addPageNum(doc)

    infoBox({
      MapType: 'L3 > L4 Openreach',
      AccessChainNo: `${i + 1} of ${cableMaps.length}`,
      Author: documentInformation.Author,
      Contact: documentInformation.Contact,
      Date: documentInformation.Date,
