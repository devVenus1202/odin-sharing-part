...
import CableDiagramWorker from "worker-loader!./cablediagrampdf.odinworker";
import './styles.scss'
...
const CableDiagram = (props: Props) => {
      ...
      await new Promise(resolve => setTimeout(resolve, 2000)).then(() => {
        graph.toFullDataURL(
          (baseImage: any) => {
            ...
            const pdfWorker = new CableDiagramWorker()
  ...
  useEffect(() => {
    ...
    if (cableDiagramData && loopChain)
      drawGraph(filterCableDiagramData(cableDiagramData, loopChain))
      ...
      title={
        <Row>
          ...
          <Col span={16} style={{ textAlign: 'right' }}>
            <Button loading={generatingPDF} disabled={generatingPDF || loadingData || !cableDiagramData.length}
                    type="default"
                  ...
                  loadingError ?
                    <div style={{ height: '78vh', width: '100%', textAlign: 'center' }}>
                      <Result
                        ...
                        status="error"
                        title="Error"
                        subTitle={`Check if Polygon with id ${polygonId} has splicing information.`}
                      />
                    </div>
                    : <></>
                }
