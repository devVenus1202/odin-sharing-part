...
class ProjectModuleRecordDetailViewWide extends React.Component<PropsType, State> {
  ...
  render() {
    ...
    let recordExternalRef: string = ''

    if (record) {
      recordType = record.type
      recordExternalRef = getProperty(record, 'ExternalRef')
    }
    ...
    const addCableDiagramAndMapViewerTabs = () => {
      if (recordExternalRef && recordType) {
        ...
        if (recordType === 'POLYGON') {
          return {
            ...
            content: {
              CableDiagram: <CableDiagram polygonId={Number(recordExternalRef)}
                                          columnFullscreen={this.state.detailsColumnExpanded}/>,
              MapViewer: <MapViewer polygonId={Number(recordExternalRef)}
                                    columnFullscreen={this.state.detailsColumnExpanded}/>,
            },
          }
        } else {
