...
export class OrdersController {
    ...
    public async createOrderItemsFromProducts(
        ...
        if ([ 'SENDGRID_ORDER_CONFIRMATION_V2', 'SENDGRID_ORDER_CONFIRMATION' ].includes(templateKey)) {
            return await this.ordersService.sendOrderEmail(principal, orderId, body);
        }

        if (templateKey === 'SENDGRID_INSTALL_SCHEDULING_REQUEST') {
            return await this.ordersService.sendInstallRequestEmail(principal, orderId, body);
        }
    }
    ...
    public async getOrderPaymentList(
        ...
        return await this.ordersService.getOrderPaymentList(principal, orderId);
    }
