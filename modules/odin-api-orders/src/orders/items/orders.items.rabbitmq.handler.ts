...
const { ORDER_ITEM, ORDER, PRODUCT } = SchemaModuleEntityTypeEnums;
...
export class OrdersItemsRabbitmqHandler {
    ...
    private async handleOrderItemUpdatedEvent(msg: IDbRecordUpdated) {
        ...
        try {
            if (msg.event === LogsConstants.DB_RECORD_UPDATED) {
                await this.ordersItemsService.recalculateRelatedOrdersByOrderItem(msg.principal, msg.id, false);
            }
        } catch (e) {
            console.error(e);
        }
    ...
    private async handleOrderItemDeletedEvent(msg: IDbRecordDeleted) {
        ...
        try {
            await this.ordersItemsService.recalculateRelatedOrdersByOrderItem(msg.principal, msg.id, true);
    ...
    private async handleOrderItemProductAssociationCreated(msg: IDbRecordAssociationCreated) {

        console.log('msg order order item association created', msg);
    ...
    @RabbitSubscribe({
        exchange: process.env.MODULE_NAME,
        routingKey: `${process.env.MODULE_NAME}.${SUB_DB_RECORD_ASSOCIATION_CREATED}`,
        queue: `${process.env.MODULE_NAME}.${ORDER_ITEM}.${PRODUCT}.${SUB_DB_RECORD_ASSOCIATION_CREATED}`,
    })
    private async handleDbRecordAssociationCreated(msg: IDbRecordAssociationCreated) {

        console.log('msg order order item product association created', msg);
        ...
        if (msg.dbRecordAssociation) {
            ...
            //Handle Order Items
            if (splitParent.entityName === ORDER_ITEM && splitChild.entityName === PRODUCT) {

                const { parentRecordId } = msg.dbRecordAssociation;

                await this.ordersItemsService.handleProductAssociationAddedToOrderItem(
                    msg.principal,
                    parentRecordId,
                );
            }
