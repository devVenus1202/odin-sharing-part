...
export class ControllerInterceptor implements NestInterceptor {
    ...
    async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
        ...
        if(request.path) {
            ...
            // validate work order if user is trying to edit it
            if (requestMethod === 'PUT' && requestParams.entityName === SchemaModuleEntityTypeEnums.WORK_ORDER) {

                await this.workOrdersService.validateWorkOrder(
                        principal,
