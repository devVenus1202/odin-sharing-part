...
export class OrganizationsUsersRbacPermissionsService {
    ...
    public async create(
        ...
        const permission = await this.getByOrganizationAndName(principal.organization, permissionCreate.name)

        if(permission) {
            return permission
        }

        const newPermission = new OrganizationUserRbacPermissionEntity();

        newPermission.organization = principal.organization;
        newPermission.name = permissionCreate.name;
        newPermission.description = permissionCreate.description;
        newPermission.type = permissionCreate.type;
