...
interface OrganizationUserEntityExtended extends OrganizationUserEntity {
    zendeskToken?: string
}
...
@ApiResponse({ status: 400, type: ExceptionType, description: 'Bad request' })
...
export class OrganizationUserEntitysController {
    ...
    public async getMyProfile(
        @Headers() headers,
    ): Promise<OrganizationUserEntityExtended> {

        console.log('getMyProfile headers', headers)
        ...
        const user: OrganizationUserEntityExtended = await this.usersService.getOrganizationUserEntityById(decoded['id']);

        console.log('userId', user?.id)

        // Generate zenesk token if user has contactId(user is customer)
        if (user.contactId) {
            const zendeskToken = jwt.sign(
                {
                    name: `${user.firstname} ${user.lastname}`,
                    email: user.email,
                    external_id: user.contactId,
                    iat: Math.floor(Date.now() / 1000),
                    exp: Math.floor(Date.now() / 1000 + OrganizationUserEntitysController.JWT_EXPIRY),
                },
                process.env.ZENDESK_JWT_TOKEN_SECRET,
            )
            user.zendeskToken = zendeskToken;
        }
        return user;
    }
    ...
    public async sendRegistrationLink(
        @Principal() principal,
        @Headers() headers,
        @Body() body: IdentityOrganizationUserSendRegistrationLink,
        @Res() response,
        @Ip() reqIp,
    ): Promise<JWTResponse> {
        if (!headers.authorization) {
        ...
        const registerToken = await this.usersService.sendRegistrationLink(principal, body, decoded['id']);
        await this.userLoginHistoryService.recordLoginEvent(
        ...
        return response.status(200).json(registerToken);
    }
    ...
    public async generateRegistrationLink(
        ...
        const jwt = await this.usersService.generateRegistrationLink(body);
        console.log(jwt);
        return response.status(200).json(jwt);
    }

    /**
     * Completing registration via link
     * @param token
     * @param body
     */
    ...
    public async completeRegistration(
        @Param('token') token: string,
        @Body() body: IdentityOrganizationUserCompleteRegistration,
        @Ip() reqIp,
    ): Promise<OrganizationUserEntity> {
        const decoded = jwt.verify(token, process.env.JWT_TOKEN_SECRET);
        const user = await this.usersService.registerAndAssignToRoleAndOrganization(
            body,
            decoded['email'],
            decoded['roleId'],
        ...
        if (user) {
            ...
            throw new ExceptionType(400, 'new user not created');
        }
    }
