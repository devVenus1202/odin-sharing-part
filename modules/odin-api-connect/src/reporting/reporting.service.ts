...
export class ReportingService {
    ...
    constructor(
        ...
        @InjectConnection('myahDb') myahConnection: Connection,
        @InjectConnection('cosmosDb') cosmosConnection: Connection,
    ) {
    ...
    public async pipelinesOverview(
        ...
        if (!!query.moduleName && !!query.entityName) {
            pgQuery = `WHERE pipe.module_name = ${query.moduleName} AND pipe.entity_name = ${query.entityName} AND dbr.deleted_at IS NULL`;
        }
        if (!!query.entityName) {
    ...
    public async premisesPassed(principal: OrganizationUserEntity) {
        ...
        try {
            ...
            const premiseStatuses = await this.cosmosConnection.query(
                `
                select s.name, l.total_units
                from ftth.status_view l
                left join ftth.build_status s on (s.id::text = l.build_status_id)
                `,
            )

            const buildDoneTotal = Number(premiseStatuses.find(elem => elem.name === '7-Build Done')['total_units'])
            const doneTotal = Number(premiseStatuses.find(elem => elem.name === '8-RFS')['total_units'])
