...
async function sync() {
    try {
        ...
        const oltIps = await pg.query(`
                select c.value as ip
        ...
        for(const { ip } of oltIps) {
            ...
            try {
                ...
                devices = devicesRes['data']['interfaces']['activatedOnus'];
            } catch (e) {
                    ...
                    if (!ontDeviceOdin[0]) {
                        ...
                        if (description) {
                            ...
                            if (order && order.length === 1) {
                                ...
                                console.log('address does not match any order', device.serialNumber);
                                // Run a health check
                            ...
                            console.log('description issue', device.description)
                            descriptionErrors.push(device.description)
                        }
                                ...
                                if (baseItem) {
                                    ...
                                    if (statusCheck) {
                                        ...
                                        updateDto.properties = {
                                            OnuInterfaceName: device.interfaceName,
                                            OltIpAddress: ip,
                                            PONPort: device.port,
                        ...
                        // check for each of the devices any that do not have an orderItem delete the record

                        console.log('ontDeviceOdin', ontDeviceOdin)

                        for(const device of ontDeviceOdin) {
                            // get the ont device and order item
                            const ontDeviceRes = await httpClient.getRequest(
                                Utilities.getBaseUrl(SERVICE_NAME.SERVICE_MODULE),
                                `v1.0/db/CustomerDeviceOnt/${device.record_id}?entities=["OrderItem"]`,
                                apiToken,
                            );
                            const ontDevice = ontDeviceRes['data'];
                            const ontDeviceOrderItem = ontDevice['OrderItem'].dbRecords;
                            if (!ontDeviceOrderItem) {
                                // delete the record
                                console.log('DELETE_ONT_DEVICE', ontDevice.title, ontDevice.id)
        ...
        console.log('devices_not_in_odin____')
        for(const item of devicesNotInOdin) {
            console.log(item)
        }
