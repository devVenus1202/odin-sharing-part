...
async function sync() {
    ...
    // Command line arguments
    let argExPolygonIds = process.argv.find(arg => arg.indexOf('expolyids') > -1);
    let exPolygonIds = argExPolygonIds ? argExPolygonIds.split('=')[1] : null;
    ...
    try {
        ...
        const data = await pg.query(
            ...
            AND EXISTS (
                ...
                WHERE a.parent_record_id = db_records.id
        ...
        for(const record of data) {
            ...
            if (orderItems && workOrder) {
                ...
                reportRow.type = workOrder.type;
                reportRow.subType = getProperty(workOrder, 'SubType');
