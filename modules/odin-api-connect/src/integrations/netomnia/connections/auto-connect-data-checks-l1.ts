...
export async function checkDataForL1Polygon(l0ClosureId: string, l1PolygonId: string, { odinDb, cosmosDb }) {
    ...
    try {
        ...
        const odinClosures = await getTotalClosuresByTypeInOdinByPolygonId(l1PolygonId, undefined, { odinDb })
        const odinCables = await getTotalCablesByTypeInOdinByPolygonId(l1PolygonId, undefined, { odinDb })
        console.timeEnd('odin_cables')
        console.time('all_closures')
        const odinClosureList = await getAllFeaturesByPolygonId(l1PolygonId, 'CLOSURE', 'L1PolygonId', { odinDb })
        const extClosureIds = odinClosureList.map(elem => Number(elem['ext_ref']))
        console.timeEnd('all_closures')
        console.time('all_cables')
        const odinCableList = await getAllFeaturesByPolygonId(l1PolygonId, 'CABLE', 'L1PolygonId', { odinDb })
        ...
        const odinCableCountRes = await getTotalCountOfFeaturesByPolygon(l1PolygonId, undefined, 'CABLE', { odinDb });
        ...
        const gisClosures = await getTotalClosuresByTypeInGisByPolygonId(l1PolygonId, { cosmosDb })
        const gisCables = await getTotalCablesByTypeInGisByPolygonId(l1PolygonId, { cosmosDb })
