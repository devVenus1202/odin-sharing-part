...
export class SchemaColumnEntity extends Base {
  ...
  @ManyToOne(type => SchemaAssociationEntity, { nullable: true })
  @JoinColumn({ name: 'linked_schema_association_id', referencedColumnName: 'id' })
  public linkedSchemaAssociation?: SchemaAssociationEntity;

  // ODN-2224 linked schema association for the LOOKUP column
  @Column({ name: 'linked_schema_association_id' })
  @RelationId((schemaColumn: SchemaColumnEntity) => schemaColumn.linkedSchemaAssociation) // you need to specify target
  ...
  /**
   * is the schema records title_case
   */
  @ApiProperty()
  @IsOptional()
  @Column({
    type: 'enum',
    enum: ChangeCaseEnums,
    nullable: true,
  })
