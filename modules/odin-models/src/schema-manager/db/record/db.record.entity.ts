...
export class DbRecordEntity extends Base {
  ...
  @ManyToMany(type => OrganizationUserGroupEntity, groups => groups.dbRecords, { eager: true })
  @JoinTable({
    name: 'db_records_groups',
    joinColumn: {
      name: 'record_id',
    },
    inverseJoinColumn: {
  ...
  /**
   * ODN-1935 - This is a unique md5 hash generated from all the unique properties
   * of a record
   *
   * concat(R2.organization_id, '::', R2.title, '::', string_agg(concat(C.column_id::text,',', c.value::text), ','
   * order by C.column_name asc), '::', R2.schema_id::text, '::', R2.schema_type_id::text)
   */
