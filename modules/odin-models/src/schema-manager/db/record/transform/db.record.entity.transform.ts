import { GROUP_NAME__ARCHIVED } from '../../../../identity/identity.constants';
...
export class DbRecordEntityTransform extends DbRecordEntityTransformBase {
  ...
  /**
   * Indicates if the record is in Archived group
   */
  ...
  public static transform(
    ...
    let transformed = {
      ...
      isArchived: dbRecord?.groups?.some(g => g.name === GROUP_NAME__ARCHIVED) ?? false,
      links: [],
    };
    ...
    if(dbRecord.childRelations || dbRecord.parentRelations) {
      ...
      const parent = dbRecord.parentRelations ? dbRecord.parentRelations.map(rel => {
        return { ...rel, id: rel.parentRecordId, dbRecordAssociationId: rel.id, relation: 'parent' }
      }) : []
      const child = dbRecord.childRelations ? dbRecord.childRelations.map(rel => {
        return { ...rel, id: rel.childRecordId, dbRecordAssociationId: rel.id, relation: 'child' }
      }) : []
    ...
    if(!!dbRecord.columns) {
      ...
      transformed = Object.assign({}, transformed, {
        ...DbRecordColumnEntityTransform.transform(dbRecord.columns, filteredCols),
      })
